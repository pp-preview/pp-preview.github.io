...
function Controls(editor) {
	...
	var row = new UIRow()
	row.setPadding("6px")
	row.setId("playButtonsRow")
	container.add(row)

	var prevButton = new UIButton()
	prevButton.setBackground("url(build/framejs/files/prev.svg)")
	...
	playButton.setBackground("url(build/framejs/files/play.svg)")
	...
	nextButton.setBackground("url(build/framejs/files/next.svg)")
	...
	fullscreenButton.setBackground("url(build/framejs/files/fullscreen.svg)")
	...
	signals.playingChanged.add(function (isPlaying) {
		playButton.setBackground(
			isPlaying
				? "url(build/framejs/files/pause.svg)"
				: "url(build/framejs/files/play.svg)"
		)
	})
